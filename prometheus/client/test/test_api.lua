...
function Test_prometheus(t)
    ...
    t:Run("gauge", function(t)
        ...
        assert(not err, err)
        assert(result.code == 200, "code:\n" .. tostring(result.code))
        assert(strings.contains(result.body, 'node_scout_nf_conntrack_insert_failed_vec{label_1="one",label_2="two"} 100', "body:\n" .. result.body))

        gauge_vec:add(1, { label_1 = "one", label_2 = "two" })
        local result, err = client:do_request(request)
        assert(not err, err)
        assert(result.code == 200, "code:\n" .. tostring(result.code))
        assert(strings.contains(result.body, 'node_scout_nf_conntrack_insert_failed_vec{label_1="one",label_2="two"} 101', "body:\n" .. result.body))

        gauge_vec:inc({ label_1 = "one", label_2 = "two" })
        local result, err = client:do_request(request)
        assert(not err, err)
        assert(result.code == 200, "code:\n" .. tostring(result.code))
        assert(strings.contains(result.body, 'node_scout_nf_conntrack_insert_failed_vec{label_1="one",label_2="two"} 102', "body:\n" .. result.body))
    end)

    t:Run("re-register gauge fails", function(t)
    ...
    t:Run("counter", function(t)
        ...
        assert(not err, err)
        assert(result.code == 200, "code:\n" .. tostring(result.code))
        assert(strings.contains(result.body, "node_scout_nf_conntrack_insert_failed_counter 1", "body:\n" .. result.body))

        counter:add(2.2)
        local result, err = client:do_request(request)
        assert(not err, err)
        assert(result.code == 200, "code:\n" .. tostring(result.code))
        assert(strings.contains(result.body, "node_scout_nf_conntrack_insert_failed_counter 3.2", "body:\n" .. result.body))
    end)
end